...
export default function GuestCategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = use(params);
  const { items, feeds, categories, readItemIds, bookmarkedItemIds, markRead, toggleBookmark } = useGuestStore();

  const category = categories.find((c) => c.id === slug);
  const categoryFeeds = feeds.filter((f) => f.category_id === slug);
  const feedIds = new Set(categoryFeeds.map((f) => f.id));
  const feedMap = new Map(feeds.map((f) => [f.id, f]));

  const enriched: FeedItem[] = useMemo(
    () =>
      items
        .filter((item) => feedIds.has(item.feed_id))
        .map((item) => ({
          ...item,
    ...
    [items, slug, readItemIds, bookmarkedItemIds]
  );

  return (
    <div>
      <div className="sticky top-0 z-10 bg-bg-primary border-b border-border px-4 py-3 flex items-center gap-3">
        <h1 className="flex-1 text-base font-semibold text-text-primary">{category?.name ?? "Category"}</h1>
        <LayoutToggle />
      </div>
      <FeedList
        items={enriched}
